                ...
                {steps.map((item) => (
                    <React.Fragment key={item.id}>
                        <div className="flex flex-col items-center ">
                            <div className={`grid  aspect-square w-[34px] place-items-center rounded-full border-[1px] ${step === item.id ? "border-yellow-50 bg-yellow-900 text-yellow-50" : "border-gray-700 bg-gray-800 text-gray-300"} ${step > item.id && "bg-yellow-50 text-yellow-50"}} `} >
                                {step > item.id ? <FaCheck/> : (item.id)}
                            </div>
                        </div>

                        {item.id !== steps.length && (
                            <div className={`h-[calc(34px/2)] w-[33%] border-dashed border-b-2 ${step > item.id ? "border-yellow-50" : "border-gray-500"} `}></div>
                        )}
                    </React.Fragment>
                ))}
            </div >
